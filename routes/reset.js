...
const rm = async () => {
  ...
  try {
    ...
    if (!fs.existsSync(sp)) {
      logs.push("⚠️ Cảnh báo: Không tìm thấy file storage, sẽ tạo nếu cần");
    }
    ...
    return { 
      success: true, 
      message: 'Reset Machine ID thành công', 
      newId: machId,
      logs: await ld(logs)
    };
  } catch (error) {
    ...
    return { 
      success: false, 
      message: `Không thể reset machine ID: ${error.message}`,
      logs: await ld(logs)
    };
  }
};
...
const bt = async () => {
  ...
  try {
    ...
    if (!fs.existsSync(dp)) {
      lg.push("❌ Lỗi: Không tìm thấy cơ sở dữ liệu SQLite");
      return { success: false, message: "Không tìm thấy cơ sở dữ liệu SQLite", logs: await ld(lg) };
    }
    ...
    return { 
      success: true, 
      message: 'Đặt lại giới hạn token thành công',
      logs: await ld(lg)
    };
  } catch (e) {
    ...
    return { 
      success: false, 
      message: `Không thể đặt lại giới hạn token: ${e.message}`,
      logs: await ld(lg)
    };
  }
};
...
const du = async () => {
  ...
  try {
    ...
    if (!fs.existsSync(sp)) {
      logs.push("❌ Lỗi: Không tìm thấy file storage");
      return { success: false, message: "Không tìm thấy file storage", logs: await ld(logs) };
    }
    ...
    return { 
      success: true, 
      message: 'Tắt cập nhật tự động thành công',
      logs: await ld(logs)
    };
  } catch (error) {
    logs.push(`❌ Error: ${error.message}`);
    return { 
      success: false, 
      message: `Không thể tắt cập nhật: ${error.message}`,
      logs: await ld(logs)
    };
  }
};
...
const ep = async () => {
  ...
  try {
    ...
    if (!fs.existsSync(dp)) {
      logs.push("❌ Lỗi: Không tìm thấy cơ sở dữ liệu SQLite");
      return { success: false, message: "Không tìm thấy cơ sở dữ liệu SQLite", logs: await ld(logs) };
    }
    ...
    return { 
      success: true, 
      message: 'Kích hoạt tính năng Pro thành công',
      logs: await ld(logs)
    };
  } catch (error) {
    logs.push(`❌ Error: ${error.message}`);
    return { 
      success: false, 
      message: `Không thể kích hoạt tính năng Pro: ${error.message}`,
      logs: await ld(logs)
    };
  }
};
        ...
        try {
          ...
          if (content.includes('Pro Trial')) {
            ...
            const newContent = content
              .replace(/Pro Trial/g, '<b>Pro</b> by PHH')
              .replace(/"Pro Trial"/g, '"Pro by PHH"')
              .replace(/'Pro Trial'/g, "'Pro by PHH'");
