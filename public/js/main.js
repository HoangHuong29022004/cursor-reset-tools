document.addEventListener('DOMContentLoaded', () => {
  ...
  const gs = async () => {
    try {
      ...
      if (data.cursor && (data.exists.machineId || data.exists.storage || data.exists.sqlite)) {
        ...
        cursorHtml += `<tr><td colspan="2">Cursor chưa được cài đặt trên hệ thống này</td></tr>`;
      }
      cursorHtml += '</table>';
      ...
      if (data.cursor && data.cursor.isRunning) {
        cursorHtml += `
          <div class="alert alert-warning">
            ...
            <strong>Cảnh báo:</strong> Cursor đang chạy. Vui lòng đóng nó trước khi sử dụng bất kỳ tính năng nào.
          </div>
        `;
      }
      ...
      if (!data.exists.machineId && !data.exists.storage && !data.exists.sqlite) {
        cursorHtml += `
          <div class="alert alert-warning">
            <i class="ri-alert-line"></i>
            <strong>Cảnh báo:</strong> Không tìm thấy file Cursor. Hãy đảm bảo Cursor được cài đặt đúng cách.
          </div>
        `;
      }
      ...
      si.innerHTML = `<div class="error"><i class="ri-error-warning-line"></i>Lỗi: ${error.message}</div>`;
      cs.innerHTML = `<div class="error"><i class="ri-error-warning-line"></i>Lỗi: ${error.message}</div>`;
    }
  };
    ...
    try {
      ...
      rr.innerHTML = `
        <div class="processing">
          <p><i class="ri-loader-2-line ri-spin"></i>Đang reset machine ID... Vui lòng chờ</p>
        </div>
      `;
      ...
      if (result.success) {
        rr.innerHTML = `
          <div class="success">
            <p><i class="ri-check-line"></i><strong>Thành công!</strong> Machine ID đã được reset thành: ${result.newId || 'ID mới'}</p>
        ...
        rr.innerHTML = `
          <div class="error">
            <p><i class="ri-error-warning-line"></i><strong>Không thể reset machine ID</strong></p>
            <p>Lỗi: ${result.message || 'Lỗi không xác định'}</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
      }
      ...
      rr.innerHTML = `<div class="error"><i class="ri-error-warning-line"></i>Lỗi: ${error.message}</div>`;
    } finally {
      rb.disabled = false;
    }
  };
  ...
  const bk = async () => {
    try {
      ...
      rr.innerHTML = `
        <div class="processing">
          <p><i class="ri-loader-2-line ri-spin"></i>Đang bỏ qua giới hạn token... Vui lòng chờ</p>
        </div>
      `;
      ...
      if (result.success) {
        rr.innerHTML = `
          <div class="success">
            <p><i class="ri-check-line"></i><strong>Thành công!</strong> Giới hạn token đã được đặt lại.</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
      } else {
        rr.innerHTML = `
          <div class="error">
            <p><i class="ri-error-warning-line"></i><strong>Không thể đặt lại giới hạn token</strong></p>
            <p>Lỗi: ${result.message || 'Lỗi không xác định'}</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
  ...
  const dz = async () => {
    try {
      ...
      rr.innerHTML = `
        <div class="processing">
          <p><i class="ri-loader-2-line ri-spin"></i>Đang tắt cập nhật tự động... Vui lòng chờ</p>
        </div>
      `;
      ...
      if (result.success) {
        rr.innerHTML = `
          <div class="success">
            <p><i class="ri-check-line"></i><strong>Thành công!</strong> Cập nhật tự động đã được tắt.</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
      } else {
        rr.innerHTML = `
          <div class="error">
            <p><i class="ri-error-warning-line"></i><strong>Không thể tắt cập nhật tự động</strong></p>
            <p>Lỗi: ${result.message || 'Lỗi không xác định'}</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
  ...
  const pt = async () => {
    try {
      ...
      rr.innerHTML = `
        <div class="processing">
          <p><i class="ri-loader-2-line ri-spin"></i>Đang chuyển đổi sang Pro... Vui lòng chờ</p>
        </div>
      `;
      ...
      if (result.success) {
        rr.innerHTML = `
          <div class="success">
            <p><i class="ri-check-line"></i><strong>Thành công!</strong> Tính năng Pro đã được kích hoạt.</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
      } else {
        rr.innerHTML = `
          <div class="error">
            <p><i class="ri-error-warning-line"></i><strong>Không thể kích hoạt tính năng Pro</strong></p>
            <p>Lỗi: ${result.message || 'Lỗi không xác định'}</p>
            ${result.logs ? `<pre class="log-output">${result.logs}</pre>` : ''}
          </div>
        `;
